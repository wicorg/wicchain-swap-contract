...
contract MonLmPoolDeployer {
    ...
    function deploy(address pool) external onlyMasterChef returns (address lmPool) {
        ...
        lmPool = address(new MonLmPool{salt: keccak256(abi.encode(pool, masterChef, block.timestamp))}());

        delete parameters;

        // Set new LMPool for pancake v3 pool.
        IMonFactory(INonfungiblePositionManager(IMasterChefV3(masterChef).nonfungiblePositionManager()).factory())
