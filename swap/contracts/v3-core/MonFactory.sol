...
/// @title Canonical MonSwap factory
/// @notice Deploys MonSwap pools and manages ownership and control over pool protocol fees
contract MonFactory is IMonFactory {
    /// @inheritdoc IMonFactory
    ...
    function createPool(
        ...
        pool = IMonPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMonPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMonPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMonPool(pool).setLmPool(lmPool);
    }
