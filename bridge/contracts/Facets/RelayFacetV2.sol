...
contract RelayFacetV2 is ILiFi, ReentrancyGuard, SwapperV2, Validatable {
    ...
    // Token address of fee bridge protocol
    address public immutable feeProtocolAddress;
    ...
    error InvalidQuote(bytes32 message, address signer);

    error NotEnoughAmount(bytes32 message);
    ...
    modifier onlyValidQuote(
        ...
        ILiFi.FeeData memory _feeData
    ) {
        // Ensure that the id isn't already consumed
        if (consumedIds[_relayData.requestId]) {

            revert InvalidQuote(_relayData.requestId, address(0));
        }
        ...
        if (
            ...
            revert InvalidQuote("2", address(0));
        }
        ...
        bytes32 message = ECDSA.toEthSignedMessageHash(
            keccak256(
                abi.encodePacked(
                    ...
                    _feeData.feeProtocolAmount,
                    _feeData.feeNetworkAmount
                )
            )
        );
        address signer = ECDSA.recover(message, _relayData.signature);
        if (signer != relaySolver) {
            revert InvalidQuote(message, signer);
        }
        _;
    }
    ...
    constructor(address _relayReceiver, address _relaySolver, address _feeProtocolAddress) {
    ...
    function startBridgeTokensViaRelay(
        ...
        RelayData calldata _relayData,
        ILiFi.FeeData calldata _feeData
    )
    external
    payable
    nonReentrant
    onlyValidQuote(_bridgeData, _relayData, _feeData)
    refundExcessNative(payable(msg.sender))
    validateBridgeData(_bridgeData)
    doesNotContainSourceSwaps(_bridgeData)
    doesNotContainDestinationCalls(_bridgeData)
    onlyValidFee(_bridgeData, _feeData)
    {
        LibAsset.depositAsset(
            _bridgeData.sendingAssetId,
            _bridgeData.minAmount
        );
        // Transfer fee protocol to
        if (_feeData.feeProtocolAmount > 0) {
            LibAsset.depositAsset(
                feeProtocolAddress,
                _feeData.feeProtocolAmount
            );
        }
        _startBridge(_bridgeData, _relayData, _feeData);
    }
    ...
    function _startBridge(
        ...
        // collection fee bridge
        _collectFee(_feeData, _relayData);
    ...
    function _collectFee(ILiFi.FeeData memory _feeData, RelayData calldata _relayData) internal {
        // Send Native networkFee to relayReceiver along with requestId as extra data,
        (bool success, bytes memory reason) = relayReceiver.call{
                value: _feeData.feeNetworkAmount
            }(abi.encode(_relayData.requestId));
        if (!success) {
            revert(LibUtil.getRevertMsg(reason));
        }

        if (LibAsset.isNativeAsset(feeProtocolAddress)) {
            // Send Native fee to relayReceiver along with requestId as extra data,
            (bool success, bytes memory reason) = relayReceiver.call{
                    value: _feeData.feeProtocolAmount
                }(abi.encode(_relayData.requestId));
            if (!success) {
                revert(LibUtil.getRevertMsg(reason));
            }
        ...
        emit FeeBridge(_feeData);
    }

    modifier onlyValidFee(
        ILiFi.BridgeData calldata _bridgeData,
        ILiFi.FeeData calldata _feeData
    ) {
        if (LibAsset.isNativeAsset(_bridgeData.sendingAssetId)) {
            // Native
            uint totalAmount = _bridgeData.minAmount + _feeData.feeNetworkAmount;
            if (totalAmount > msg.value) {
                revert NotEnoughAmount("Insufficient value");
            }
        } else {
            if (_feeData.feeNetworkAmount > msg.value) {
                revert NotEnoughAmount("Insufficient value");
            }
        }
        _;
